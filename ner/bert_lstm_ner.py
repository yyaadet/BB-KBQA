...
# -*- coding:utf-8 -*-
"""
reference from: https://github.com/macanv/BERT-BiLSTM-CRF-NER
"""
...
from ner.models import create_model, InputFeatures, InputExample
...
class NerProcessor(DataProcessor):
    ...
    def get_test_examples(self, data_dir):
        return self._create_example(
            self._read_data(os.path.join(data_dir, "test9870.txt")), "test")
    ...
    def get_labels(self, labels=None):
        ...
        if os.path.exists(os.path.join(self.output_dir, 'label_list.pkl')):
            ...
            if len(self.labels) > 0:
                ...
                self.labels = ["O", 'B-entity', 'I-entity', "X", "[CLS]", "[SEP]"]
        return self.labels
    ...
    if args.do_predict:
        ...
        def result_to_pair(writer):
            for predict_line, prediction in zip(predict_examples, result):
                ...
                idx2cnt = dict()
                cnt = 1
                for i, word in enumerate(line_token):
                    token = tokenizer.tokenize(word)
                    idx2cnt[i] = cnt
                    cnt += len(token)
                for i in range(len(line_token)):
                    cur_label = id2label[prediction[idx2cnt[i]]]
                    line += line_token[i] + ' ' + label_token[i] + ' ' + cur_label + '\n'
